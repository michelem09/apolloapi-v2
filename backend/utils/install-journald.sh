...
#
# Usage: install-journald.sh [APOLLO_DIR]
#
# Takes the tree as an argument: callers set APOLLO_DIR as a plain (unexported)
# shell variable, which a child `bash` would never inherit — so relying on the
# environment silently used the default instead of the caller's tree.
set -e
APOLLO_DIR="${1:-${APOLLO_DIR:-/opt/apolloapi}}"
